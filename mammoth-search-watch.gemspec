...
Gem::Specification.new do |spec|
  ...
  spec.summary = "SERP observation and drift capture with embedded Mammoth by default."
  spec.description = <<~DESCRIPTION.strip
    Mammoth Search Watch captures SERP request and response observations,
    persists them as PostgreSQL facts, and embeds Mammoth by default to deliver
    resulting WAL-backed change events. Sink-only mode is supported as an
    explicit opt-out. It is intentionally PostgreSQL-first, WAL-centric, and built
    as a Mammoth descendant rather than a generic HTTP event bus.
  DESCRIPTION
